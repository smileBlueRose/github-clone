...
from domain.ports.repositories.git_repo import (
    AbstractGitRepoReadRepository,
    AbstractGitRepoWriteRepository,
)
...
class GitRepoWriteRepository(AbstractGitRepoWriteRepository):
    ...
    async def update(self, identity: UUID, schema: GitRepoUpdateSchema) -> Repository:
        ...
        if repo_model is None:
            raise GitRepositoryNotFoundException(repo_id=identity)
...
class GitRepoReadRepository(AbstractGitRepoReadRepository):
    def __init__(self, session: AsyncSession) -> None:
        self._session = session
